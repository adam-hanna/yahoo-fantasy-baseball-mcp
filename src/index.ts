...
class YahooFantasyBaseballServer {
   ...
   */
  private async yahooGet(url: string, params?: Record<string, string>) {
    try {
  ...
  private async yahooPost(url: string, xmlBody: string) {
  ...
  private async yahooPut(url: string, xmlBody: string) {
    const headers = { 'Authorization': `Bearer ${accessToken}`, 'Content-Type': 'application/xml' };
    try {
   ...
   */
  private flattenPlayerInfo(infoArray: any[]): Record<string, any> {
  ...
  private parsePlayersFromResponse(playersObj: any): any[] {
    return Object.values(playersObj)
      .filter((item: any) => typeof item === 'object' && item.player)
      .map((item: any) => {
        ...
        return {
          ...
          eligible_positions: (info.eligible_positions || []).map((p: any) => p.position),
          stats: Array.isArray(statsSection)
            ? statsSection.map((s: any) => ({ stat_id: s.stat?.stat_id, value: s.stat?.value }))
            : [],
        };
      });
  }
  ...
  private async getLeagueSettings() {
    ...
    const roster_positions = (settings.roster_positions || []).map((rp: any) => ({
      position: rp.roster_position.position,
      count: rp.roster_position.count,
    }));

    const stat_categories = (settings.stat_categories?.stats || []).map((s: any) => ({
    ...
    const stat_modifiers = (settings.stat_modifiers?.stats || []).map((s: any) => ({
      stat_id: s.stat.stat_id,
      value: s.stat.value,
    }));
  ...
  private async getLeagueTeams() {
    ...
    const teams = Object.values(teamsObj)
      .filter((item: any) => typeof item === 'object' && item.team)
      .map((item: any) => {
  ...
  private async getDraftResults() {
    ...
    const draft_results = Object.values(draftsObj)
      .filter((item: any) => typeof item === 'object' && item.draft_result)
      .map((item: any) => ({
  ...
  private async getAvailablePlayers(args: any) {
    let url = `/league/${LEAGUE_KEY}/players;status=A`;
    if (args?.position) url += `;position=${args.position}`;
    if (args?.sort) url += `;sort=${args.sort}`;
    if (args?.sort_type) url += `;sort_type=${args.sort_type}`;
    if (args?.sort_season) url += `;sort_season=${args.sort_season}`;

    const params: Record<string, string> = {};
    if (args?.start != null) params.start = String(args.start);
  ...
  private async searchPlayers(args: any) {
  ...
  private async getPlayerStats(args: any) {
    ...
    if (args?.stat_type) url += `;type=${args.stat_type}`;
    if (args?.season) url += `;season=${args.season}`;

    const response = await this.yahooGet(url);
    const player = response.data.fantasy_content.player;
    const info = this.flattenPlayerInfo(player[0]);
    const stats = (player[1]?.player_stats?.stats || []).map((s: any) => ({
      stat_id: s.stat?.stat_id,
      value: s.stat?.value,
    }));
      ...
      .map((item: any) => {
        ...
        const players = playersObj
          ? Object.values(playersObj)
              .filter((p: any) => typeof p === 'object' && p.player)
              .map((p: any) => {
  ...
  private async addDropPlayer(args: any) {
